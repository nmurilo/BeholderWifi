 ...
 * Original Copyright (c) 1997-2004 Jean Tourrilhes <jt@hpl.hp.com>
...
extern struct wireless_scan *ap_table_init;
extern struct wireless_scan *wscan_init;
...
const char * const iw_operation_mode[] = { "AT",
					"ad",
					"AP",
					"AP",
					"RP",
		 ...
		 const struct timezone *	tz)
{
	...
	s = (time->tv_sec - tz->tz_minuteswest * 60) % 86400;
			...
			struct wireless_scan *		wscan)
{
   ...
   switch(event->cmd)
   {
      case SIOCGIWAP:
         ...
         if (ap_table_init == NULL) /* First time */ 
            ap_table_init = wscan; 
        if (wscan_init == NULL) /* First time */ 
            wscan_init = wscan; 
      ...
      case SIOCGIWESSID:
         ...
         wscan->b.has_essid = 0;
//         event->u.essid.length = strlen(event->u.essid.pointer)+1;
         if((event->u.essid.pointer) && (event->u.essid.length))
         {
            if (event->u.essid.length > 1 && *event->u.essid.pointer)
            {
               wscan->b.has_essid = 1;
               memcpy(wscan->b.essid, event->u.essid.pointer, event->u.essid.length);
               wscan->b.essid[event->u.essid.length] = '\0';
// printf("essid = %s, event->u.essid.length = %d, len = %d\n", 
//         wscan->b.essid, event->u.essid.length, strlen(wscan->b.essid));
            }
         }
         break;
      case SIOCGIWENCODE:
